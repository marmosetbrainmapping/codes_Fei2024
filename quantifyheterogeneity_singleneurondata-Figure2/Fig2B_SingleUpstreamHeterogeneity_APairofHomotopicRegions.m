...
rand_heterogeneity = zeros(length(PFCregions),length(Cortexregions));
norm_heterogeneity = zeros(length(PFCregions),length(Cortexregions));
...
for i = 1:size(data_heterogeneity,1)
    ...
    for j = 1:size(data_heterogeneity,2)
        ...
        randoverlap = p1*p2/(min(p1,p2));
        data_heterogeneity(i,j) = 1-overlap;
        rand_heterogeneity(i,j) = 1-randoverlap;
        norm_heterogeneity(i,j) = 1-(overlap-randoverlap);
