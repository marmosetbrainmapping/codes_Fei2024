...
% set(gca,'Color','none');
% %% test model overlap
% ohomo = zeros(size(overlap_homos));
% ohetero = zeros(size(overlap_heteros));
% 
% for i = 1:Nregion
%     selfinds = projectingneurons{i,i};
%     contrainds = projectingneurons{i+Nregion,i};
%     overlap1 = length(intersect(selfinds,contrainds))/length(contrainds);
% 
%     selfinds = projectingneurons{i+Nregion,i+Nregion};
%     contrainds = projectingneurons{i,i+Nregion};
%     overlap2 = length(intersect(selfinds,contrainds))/length(contrainds);
%     ohomo(i) = (overlap1+overlap2)/2;
% 
%     
%     for j = 1:Nregion
%         if j == i
%             continue
%         else
%             heteroinds1 = projectingneurons{j,i};
%             heteroinds2 = projectingneurons{j+Nregion,i};
%             overlap1 = length(intersect(heteroinds2,heteroinds1))/length(heteroinds1);
% 
%             heteroinds1 = projectingneurons{j,i+Nregion};
%             heteroinds2 = projectingneurons{j+Nregion,i+Nregion};
%             overlap2 = length(intersect(heteroinds2,heteroinds1))/length(heteroinds1);
%         end
%         ohetero(j,i) = (overlap1+overlap2)/2;
%     end
% end
% ohetero(logical(eye(Nregion))) = nan;

%%
niter = 100*5;
corr_thres = 0.95;

Cmeans = zeros(niter,length(noises),length(overlaphetero_range));
Cstds = Cmeans;
psyncs = Cmeans;
...
%     tic
    ...
    Ws = zeros(N*Nregion*2,N*Nregion*2,length(overlaphetero_range));
    for i = 1:length(overlaphetero_range)
        overlap_homos = ones(1,Nregion)*overlaphomo;
        overlap_heteros = ones(Nregion,Nregion)*overlaphetero_range(i);
...
%     tic
    ...
    Cmean = zeros(length(noises),length(overlaphetero_range));
    Cstd = Cmean;
    psync = Cmean;
    for i = 1:length(noises)
        ...
        for j = 1:length(overlaphetero_range)
            W = squeeze(Ws(:,:,j));
...
toc
%%
save(['/zfs/scratch/qihang/HeteroParams_Nregion',num2str(Nregion),'.mat']);
